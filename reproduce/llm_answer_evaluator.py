...
"""
    ...
    def create_fallback_evaluation(self, response: str) -> Dict[str, Any]:
        ...
        self.logger.info(f"Created fallback evaluation result: accuracy={fallback_result['accuracy']}")
    ...
    async def evaluate_single_answer(self, question: str, expected_answer: str,
        ...
        try:
            ...
            try:
                ...
                if json_code_block_match:
                    ...
                    try:
                        ...
                        self.logger.warning(f"Failed to parse JSON inside code block: {e}")
                        ...
                        try:
                            ...
                            self.logger.warning("JSON repair failed; using fallback evaluation result")
            ...
            evaluation_time = time.time() - start_time
            error_msg = f"Evaluation timed out after {self.config.request_timeout} seconds"
    ...
    async def evaluate_multiple_results(self, results_files: List[str], evaluation_type: str = "comprehensive",
        ...
        self.logger.info(f"Starting evaluation for multiple result files: {results_files}")
        ...
        if max_evaluations:
            for file_type in all_results_by_type:
                ...
                self.logger.info(f"Limited {file_type} results: {original_count} -> {limited_count}")
        ...
        if self.existing_evaluations:
            ...
            self.logger.info(f"Found {existing_count} existing LLM evaluations; duplicate evaluations will be skipped")
        ...
        for i, result in enumerate(all_results, 1):
            ...
            if self.has_any_error(result):
                skipped_count += 1
                self.logger.info(f"  Skipping query with errors: {self.get_error_summary(result)}")
                continue
            ...
            if result["_file_type"] == "vlm":
                ...
                if result.get("vlm_result") and result["vlm_result"].get("success"):
                    ...
                    if self.is_evaluation_completed(result["doc_id"], result["question"], method):
                        ...
                        if existing_eval:
                            existing_eval["source_file"] = result["_source_file"]  # Refresh source file metadata.
            ...
            if i % 10 == 0:
                ...
                self.logger.info(f"Saved the first {len(self.evaluation_results)} evaluation results")
    ...
    async def evaluate_vlm_results(self, vlm_results: List[Dict], evaluation_type: str):
        ...
        for i, vlm_result in enumerate(vlm_results, 1):
            ...
            if self.has_any_error(vlm_result):
                skipped_count += 1
                self.logger.info(f"  Skipping VLM query with errors: {self.get_error_summary(vlm_result)}")
                continue
    ...
    async def evaluate_standard_rag_results(self, rag_results: List[Dict], evaluation_type: str):
        ...
        for i, rag_result in enumerate(rag_results, 1):
            ...
            if self.has_any_error(rag_result):
                skipped_count += 1
                self.logger.info(f"  Skipping RAG query with errors: {self.get_error_summary(rag_result)}")
                continue
    ...
    def generate_evaluation_report(self, total_time: float):
        ...
        if not successful_evaluations:
            self.logger.warning("No successful evaluation results were found, so the report cannot be generated")
            return
...
"""
        ...
        for result in successful_evaluations:
            ...
            folder_name = os.path.basename(os.path.dirname(source_file_path)) if source_file_path else ""
            # Build a display doc ID prefixed with the folder identifier when available.
            display_doc_id = f"{folder_name}/{original_doc_id}" if folder_name else original_doc_id
...
"""
                for doc_id_display, methods in multi_method_docs.items():
                    for method, results in methods.items():
                        ...
                        short_doc_id = doc_id_display[:30] + "..." if len(doc_id_display) > 30 else doc_id_display
...
"""
                ...
                for doc_id_display, methods in single_method_docs.items():
                    if primary_method in methods:
                        ...
                        for r in results:
                            ...
                            elif any(keyword in doc_id_display.lower() for keyword in ["manual", "guide", "handbook"]):
                                doc_types.add("Manual / guide")
                            elif any(keyword in doc_id_display.lower() for keyword in ["report", "survey"]):
                ...
                doc_performance.sort(key=lambda x: (_folder_num_from_doc_id(x["doc_id"]), x["doc_id"]))
...
"""
        ...
        self.logger.info(f"Detailed results: {os.path.join(self.config.output_dir, 'llm_evaluation_results.json')}")
        self.logger.info(f"Summary: {os.path.join(self.config.output_dir, 'llm_evaluation_summary.csv')}")
    ...
    def load_existing_evaluations(self):
        ...
        if os.path.exists(results_file):
            try:
                ...
                self.logger.info(f"Loaded {len(existing_data)} existing LLM evaluation results")
                ...
                self.logger.info(f"Built a lookup index for {len(self.existing_evaluations)} LLM evaluation results")
                
            except Exception as e:
                self.logger.warning(f"Error loading existing LLM evaluation results: {e}; starting from scratch")
                self.evaluation_results = []
                self.existing_evaluations = {}
        else:
            self.logger.info("No existing LLM evaluation results file found; starting from scratch")
    ...
    def update_evaluation_results_list(self, new_results: List[Dict[str, Any]]):
        ...
        self.logger.debug(f"Updated LLM evaluation results; current total count: {len(self.evaluation_results)}")
    ...
    async def evaluate_qa_files(self, qa_files: List[str], evaluation_type: str = "comprehensive",
        ...
        for file_idx, qa_file in enumerate(qa_files, 1):
            ...
            if not isinstance(qa_items, list):
                self.logger.warning(f"Invalid QA file format (expected an array), skipped: {qa_file}")
                continue
    ...
    async def evaluate_qa_directory(self, qa_data_dir: str, evaluation_type: str = "comprehensive",
                                   max_evaluations: Optional[int] = None) -> None:
        ...
        if not qa_files:
            self.logger.warning("No QA result files found matching qa_results_naive_mm*.json")
            return
...
def main():
    ...
    try:
        if config.rag_results_files:
            ...
            asyncio.run(evaluator.evaluate_qa_directory(
                config.qa_data_dir,
                config.evaluation_type,
                config.max_evaluations
            ))
